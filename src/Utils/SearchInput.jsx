...
const SearchBox = () => {
  ...
  const filterData = () => {
    ...
    const filteredTopics = dataTopicsSearch?.filter((topic) =>
      topic.title?.toLowerCase().includes(search)
    ) || [];
    return { filteredSections, filteredPreliminaries, filteredTopics };
  };

  const { filteredSections, filteredPreliminaries, filteredTopics } = filterData();
  ...
  const highlightText = (text) => {
    ...
    return parts.map((part, index) =>
      regex.test(part) ? (
        <span
          key={index}
          style={{ fontWeight: "bold", textDecoration: "underline", color: "orange" }}
        >
          {part}
        </span>
      ) : (
    ...
    >
      ...
      <AnimatePresence>
        {searchTerm && (
          ...
          filteredPreliminaries.length > 0 ||
            ...
            >
              ...
              {filteredTopics.map((topic) => (
                <li key={`topic-${topic.id}`}>
                  <Link href={`/preliminaries/${topic.slug}`} className={styles.dropdownItem}>
